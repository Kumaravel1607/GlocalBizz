...
class Edit_AddressPage extends StatefulWidget {
  final String addressID;
  Edit_AddressPage({Key key, this.addressID}) : super(key: key);

  @override
  _Edit_AddressPageState createState() => _Edit_AddressPageState();
}

class _Edit_AddressPageState extends State<Edit_AddressPage> {
  ...
  address_detail() async {
    ...
    var response = await http
        .post(Uri.parse(api_url + '/customer_address_view'), body: data);
    if (response.statusCode == 200) {
      jsonResponse = json.decode(response.body);
      nameController.text = jsonResponse['first_name'];
      lastnameController.text = jsonResponse['last_name'];
      emailController.text = jsonResponse['email'];
      mobileController.text = jsonResponse['mobile_no'];
      addressController.text = jsonResponse['address1'];
      addressController2.text = jsonResponse['address2'];
      cityController.text = jsonResponse['city'];
      landmarkController.text = jsonResponse['landmark'];
      postalController.text = jsonResponse['pincode'];
      setState(() {
        addressType = jsonResponse['address_type'];
        _isLoading = true;
      });
    } else {
      // setState(() {
  ...
  void _updateAddress(
    ...
    var jsonResponse;
    var response =
        await http.post(Uri.parse(api_url + "/update_address"), body: data);
